...
class EmissionsPeakTest:
    ...
    def load_historical_data(
            ...
            year_range: range = range(1970,2020)) -> 'EmissionsPeakTest':
        """
        ...
        if year_col not in data.columns or emissions_col not in data.columns:
            raise ValueError(f"Data must contain '{year_col}' and '{emissions_col}' columns")
        
        # Standardize column names
        data = data[[year_col, emissions_col]].rename(
    ...
    def characterize_noise(
            ...
            distribution: str = 'normal') -> 'EmissionsPeakTest':
        """
        ...
        self.residuals, self.trend_info = self._calculate_residuals(method, segment_length)
        self.noise_params, self.noise_generator = self._fit_noise_distribution(distribution)
    ...
    def _calculate_residuals(self, method: str, segment_length: int) -> Tuple[np.ndarray, Dict]:
    ...
    def _fit_noise_distribution(self, noise_type='auto'):
        ...
        if self.residuals is None:
            raise ValueError("Must call characterize_noise() first to calculate residuals")
        ...
        if noise_type == 'auto':
            ...
            if normal_aic <= t_aic:
                noise_type = 'normal'
                print(f"Auto-selected normal distribution (AIC: {normal_aic:.1f} vs {t_aic:.1f})")
            else:
                noise_type = 't'
                print(f"Auto-selected t-distribution (AIC: {t_aic:.1f} vs {normal_aic:.1f})")
        ...
        if noise_type == 'normal':
            ...
            params = {
                ...
                'sigma': params_tuple[2],   # Use scale as sigma equivalent for compatibility
                'fitted_params': params_tuple
            }
    ...
    def _generate_bootstrap_slopes(self, n_bootstrap: int) -> np.ndarray:
        ...
        for _ in range(n_bootstrap):
            ...
            null_emissions = np.full(n_points, baseline_emissions) + \
    ...
    def _calculate_p_value(self, bootstrap_slopes: np.ndarray, observed_slope: float) -> float:
    ...
    def interpret_results(self, verbose: bool = True) -> Dict[str, str]:
        ...
        if verbose:
            ...
            print(f"Statistical evidence: {significance} (p = {interpretation['p_value']})")
            print(f"Conclusion: {peak_conclusion}")
            print("="*50)
        
        return interpretation
    
    def plot_analysis(self, figsize: Tuple[int, int] = (15, 10), save_path: Optional[str] = None) -> plt.Figure:
        """
    ...
    def _plot_historical_and_test_data(self, ax: plt.Axes) -> None:
        ...
        ax.plot(self.test_data['year'], self.test_data['emissions'], 
                'k+', linewidth=1, markersize=5, label='Recent test data')
    ...
    def _plot_noise_distribution(self, ax: plt.Axes) -> None:
        ...
        ax.hist(self.residuals, bins=30, density=True, alpha=0.7, color='skyblue', 
    ...
    def _plot_bootstrap_results(self, ax: plt.Axes) -> None:
        ...
        ax.hist(self.bootstrap_results['bootstrap_slopes'], bins=50, density=True, 
                alpha=0.7, color='lightgreen', label='Bootstrap slopes\n(null hypothesis)')
